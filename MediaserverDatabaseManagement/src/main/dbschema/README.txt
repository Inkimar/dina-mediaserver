...
4) Create all tables ( including demo-inserts)
4.1) Execute the 'database project by running : $mvn clean install

EJB-Project:
1) If deploying in Glassfish check the file glassfish-resources.xml
1.1) set URL to database
1.2) <property name="URL" value="jdbc:mysql://localhost:3306/nrm_demo_media"/>

DEMO, only MediaServer : Skata : ON HP
