...
import se.nrm.mediaserver.util.MediaURL;
...
public class MediaResourceForm {
    ...
    private final MediaService bean = JNDIFetchRemote.outreach();
    ...
    public boolean deleteFileFromFS(@PathParam("mediaUUID") String mediaUUID) {
        String filePath = PathHelper.getDynamicPathToFile(mediaUUID, "");
    ...
    /**
     * @param form
     * @return a unique UUID that identifies the file in the database&FS
...
//                break;
    ...
    public String getTest(@PathParam("name") String name) {
        logger.info("first info and the debug ?");
        logger.debug("DEBUG - testing log4j - : Incoming -> : " + name);

        String toUpperCase = name.toUpperCase();
        logger.debug("debug - testing log4j - : Outgoing -> : " + toUpperCase);
        return toUpperCase;
    }
}
